    ...
    useEffect(() => {
        const fetchChats = async () => {
            const response = await fetch(
                `https://journalon-api.vercel.app/api/list_chats?passcode=${passcode}`
            );
                            ...
                            <TableCell>
                                <Stack direction="row" spacing={1}>
                                    <Button
                                        variant="outlined"
                                        onClick={() => {
                                            navigate(`/journalon/chat?chatId=${chat.chatId}`);
                                        }}
                                    >
                                        Open
                                    </Button>
                                    {storedChatKeys[chat.chatId]?.chatKey && !chat.finalized && <Button
                                        ...
                                        onClick={async () => {
                                            ...
                                            const response = await fetch(
                                                `https://journalon-api.vercel.app/api/delete_chat?chatId=${chat.chatId}&chatKey=${chatKey}&passcode=${passcode}`,
                                                {
                                                    method: 'DELETE'
                                                }
                                            );
